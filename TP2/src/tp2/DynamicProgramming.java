...
public class DynamicProgramming {
    ...
    public void calculaCaminhoMinimo(){  //metodo recursivo para calcular o caminho mínimo entre a entrada e saída
        ...
        if(f1[i-2] + A1.getCustosEstacoes()[A1.getCustosEstacoes().length - 1] <= f2[i-2] + A2.getCustosEstacoes()[A2.getCustosEstacoes().length - 1]){
            fFinal = f1[i-2] + A1.getCustosEstacoes()[A1.getCustosEstacoes().length - 1];
            lFinal = 1;
        }else{
            fFinal = f2[i-2] + A2.getCustosEstacoes()[A2.getCustosEstacoes().length - 1];
            lFinal = 2;
        }
    ...
    public void printaCaminhoMinimo(){
        ...
        System.out.println("Linha: " + j + "   Estação: " + (A1.getCustosEstacoes().length - 2));
